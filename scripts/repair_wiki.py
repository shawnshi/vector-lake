...
import sys
import yaml
# Performance optimization: Use libyaml-based C loaders/dumpers (7-10x faster) when available.
# Accelerates batch frontmatter repair scripts.
try:
...
def rebuild_content(content, fm_data, fm_match):
    ...
    new_fm = yaml.dump(fm_data, Dumper=Dumper, allow_unicode=True, default_flow_style=False).strip()
    return "---\n" + new_fm + "\n---" + after_fm
