...
def visualize_vector_lake():
    ...
    # Mitigate Server-Side XSS by escaping HTML characters in JSON string
    escaped_graph_data = json.dumps(graph_data, ensure_ascii=False).replace('<', r'\u003c').replace('>', r'\u003e').replace('&', r'\u0026')
    html = html.replace("%%GRAPH_DATA%%", escaped_graph_data)
