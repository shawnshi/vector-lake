...
def visualize_vector_lake():
    ...
    # Safely escape HTML characters to prevent XSS when injecting JSON into <script>
    safe_json = json.dumps(graph_data, ensure_ascii=False).replace('<', '\\u003c').replace('>', '\\u003e').replace('&', '\\u0026')
