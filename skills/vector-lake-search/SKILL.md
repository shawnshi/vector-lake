...
<execution_workflow>
  <workflow>
    ...
    2. Determine the optimal `top_k` and `mode` ('page', 'memory', or 'claim') for the query. Apply advanced filters when appropriate: `domain` (e.g. 'HIT', 'Clinical'), `cluster`, or `include_history=True` for historical/decayed nodes.
