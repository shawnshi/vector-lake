...
<execution_workflow>
  <workflow>
    ...
    4. **Vector Lake Registry**: If historical tracking is needed, use `invoke_subagent` or `vector-lake-mcp` tools (like `update_operational_memory`), or the `memory-update` skill, to register the current debt state.
  </workflow>
