...
def test_command_inventory_and_mutation_authority_contract_are_exact():
    observed = {
        path.stem for path in (ROOT / "commands").glob("*.toml")
    }
    assert observed == EXPECTED_COMMANDS
    required_tokens = {
        "audit": ("dry_run=true", "fingerprint", "explicit", "confirmation"),
        "daemon_watchdog": (
            "mcp_runtime_status",
            "stale=false",
            "source_root",
        ...
        "delete": ("dry_run=true", "confirms", "dry_run=false"),
        "gc": (
            "dry_run=true",
            "fingerprint",
            "orphan_confirmation",
        ...
        "governance_merge": ("enqueue=false", "explicitly", "enqueue=true"),
        "graph": ("approve", "output_dir", "exists"),
        "lint": ("auto_fix=false", "explicitly", "auto_fix=true"),
        "query": ("dry_run=true", "strictly read-only", "never"),
        "research": ("dry_run=true", "explicit", "dry_run=false"),
        "resolve": ("review_governance_list", "explicitly", "exact item"),
        "review": ("dry_run=true", "explicit", "dry_run=false"),
        "search": ("'page'", "'memory'", "'fact'", "deprecated"),
        "sync": ("scan/enqueue", "does not generate or finalize"),
    }
    for name, tokens in required_tokens.items():
        ...
        assert all(token.casefold() in contract for token in tokens), name
...
def test_gc_command_binds_apply_to_the_confirmed_preview_fingerprint():
    contract = _load_command("gc")["prompt"].casefold()

    assert "exact candidate set" in contract
    assert "unchanged fingerprint" in contract
    assert "orphan_confirmation" in contract
    assert "discard the earlier confirmation" in contract


def test_query_command_is_strictly_read_only():
    contract = _load_command("query")["prompt"].casefold()

    assert "dry_run=true" in contract
    assert "strictly read-only" in contract
    assert "dry_run=false" not in contract
    assert "never request or perform persistence" in contract


def test_search_command_uses_fact_and_marks_claim_as_compatibility_only():
    contract = _load_command("search")["prompt"]

    assert "('page', 'memory', or 'fact')" in contract
    assert "deprecated compatibility alias for `fact`" in contract
    assert "fact-only operational memory" in contract
    assert "not canonical Claim records" in contract
    assert "('page', 'memory', or 'claim')" not in contract
...
def test_governance_commands_reference_registered_mcp_names():
    audit = _load_command("governance_audit")
    debt = _load_command("governance_debt")

    assert "trigger_audit_graph" in audit["prompt"]
    assert "get_governance_debt" in debt["prompt"]
    assert callable(mcp_server.trigger_audit_graph)
    assert callable(mcp_server.get_governance_debt)


def test_sync_command_reports_scan_enqueue_without_completion_claim():
    sync = _load_command("sync")

    assert "scan/enqueue" in sync["prompt"]
    assert "do not claim" in sync["prompt"]
    assert "does not generate or finalize Wiki pages" in sync["description"]
    assert callable(mcp_server.sync_vector_lake)
