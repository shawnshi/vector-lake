...
def committed_graph(isolated_memory, tmp_path, monkeypatch):
    ...
    index = {
        "nodes": {
            "Concept_One": {"title": "One", "links": ["Concept_Two"], "summary": "</script><img src=x onerror=alert(1)>"},
            "Concept_Two": {"title": "Two", "links": []},
        },
        "weighted_edges": [{"source": "Concept_One", "target": "Concept_Two", "weight": 1}],
        "governance_metrics": {"validity_state_counts": {"unknown": "<img src=x onerror=alert(2)>"}},
    }
    claims = {"nodes": [{"id": "claim_one", "name": "Synthetic claim"}], "edges": []}
    prepared = fmt.build_projection_roots(
        wiki, index, claims,
        canonical_generation=fmt._runtime_current_generations(db_store.get_connection()),
    )
...
def test_graph_export_committed_pair_is_read_only_and_meaningful(committed_graph, monkeypatch):
    ...
    assert metadata["canonical_generation"]["runtime_generations"] == prepared.canonical_generation
...
@pytest.mark.parametrize("damage", ["index_missing", "claim_missing", "sidecar_missing", "sidecar_corrupt", "object_corrupt", "stale"])
def test_graph_export_projection_failure_has_no_artifact(committed_graph, monkeypatch, damage):
    wiki, output, prepared = committed_graph
...
def test_graph_export_unconfigured_output_rejected_before_read(tmp_path, monkeypatch, roots, explicit):
    if roots is None:
        monkeypatch.delenv("VECTOR_LAKE_AGENT_SANDBOX_ROOTS", raising=False)
    else:
        monkeypatch.setenv("VECTOR_LAKE_AGENT_SANDBOX_ROOTS", roots)
    monkeypatch.setattr(tool_graph, "_read_projection_pair", lambda *a: pytest.fail("read before authorization"))
    output = tmp_path / "output"
    for call in (tool_graph.visualize_vector_lake, mcp_server.visualize_vector_lake):
        assert "VECTOR_LAKE_AGENT_SANDBOX_ROOTS" in call(str(output) if explicit else None)
    assert not output.exists()


@pytest.mark.parametrize("kind", ["empty", "whitespace", "relative", "traversal", "outside", "file", "target_directory", "invalid_root"])
def test_graph_output_rejects_unsafe_paths_without_side_effects(tmp_path, monkeypatch, kind):
    root = tmp_path / "approved"
    root.mkdir()
    monkeypatch.setenv("VECTOR_LAKE_AGENT_SANDBOX_ROOTS", str(root))
    value = {"empty": "", "whitespace": " ", "relative": "relative", "traversal": str(root / ".." / "approved" / "new"), "outside": str(tmp_path / "outside")}.get(kind, str(root))
    if kind == "file":
        value = str(root / "file")
...
def test_graph_output_rejects_symlinks(tmp_path, monkeypatch, kind):
    ...
    monkeypatch.setenv("VECTOR_LAKE_AGENT_SANDBOX_ROOTS", str(link if kind == "root" else root))
...
def test_graph_output_default_and_multiple_roots(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("VECTOR_LAKE_AGENT_SANDBOX_ROOTS", os.pathsep.join(map(str, roots)))
    assert tool_graph._graph_output_path() == roots[0] / "vector_lake_graph.html"
    assert tool_graph._graph_output_path(str(roots[1] / "sub")) == roots[1] / "sub" / "vector_lake_graph.html"
    assert not any(root.exists() for root in roots)


def test_graph_export_browser_exception_does_not_hide_saved_file(committed_graph, monkeypatch):
...
def test_graph_export_uses_heavy_executor_without_canonical_gate(committed_graph, tmp_path, monkeypatch):
    ...
    try:
        with heavy_task("maintenance", "unrelated-owner", origin="pytest", wait_timeout_seconds=0):
            before = _file_hashes(wiki)
            _forbid_mutations(monkeypatch)
...
def test_graph_renderer_no_network_until_consent(tmp_path):
    html = (Path(__file__).resolve().parents[1] / "templates" / "topology.html").read_text(encoding="utf-8")
    assert not re.search(r'<(?:script|img|iframe|link)[^>]+(?:src|href)\s*=', html, re.I)
    ...
    script = script.replace("%%GRAPH_DATA%%", '{"exportMetadata":{"projection_generation":"synthetic"}}').replace("%%MEMORY_BASE_PATH%%", '"file:///synthetic/"')
    harness = """
...
"""
    ...
    result = subprocess.run([node, str(runner), str(js)], capture_output=True, text=True, timeout=10)
...
def test_graph_export_live_wal_is_read_only(committed_graph, monkeypatch):
    ...
    before = {name: digest for name, digest in _file_hashes(wiki).items() if not name.endswith("-shm")}
    _forbid_mutations(monkeypatch)
    result = tool_graph.visualize_vector_lake()
    assert result.startswith("Saved graph:"), result
    after = {name: digest for name, digest in _file_hashes(wiki).items() if not name.endswith("-shm")}
    assert after == before


def test_graph_export_cancellation_before_publish_preserves_prior(committed_graph, monkeypatch):
    ...
    operation = CancellationRegistry().create(tool_name="visualize_vector_lake", lane="heavy", deadline=None)
...
graph = new Proxy({}, {get(target, name) {
    ...
    return (...args) => {
        if (!args.length) return settings.get(name);
...
def _run_initialized_graph_check(tmp_path, assertions, root_name="Graph Memory"):
    node = shutil.which("node")
    if not node:
    ...
    page_nodes = [{"id": "Concept_" + name, "name": name, "group": "Concept",
                   "sources": ["raw/" + name + "/note " + name + ".md"]} for name in names]
    claim_nodes = [{"id": "claim_" + name, "name": name, "group": "Claim",
                    "sources": ["Source_" + name + ".md"]} for name in names]
    payload = {
        "pageGraph": {"nodes": page_nodes, "edges": [{"source": page_nodes[1]["id"], "target": page_nodes[2]["id"]}]},
        "claimGraph": {"nodes": claim_nodes, "edges": []},
        "exportMetadata": {"projection_generation": "synthetic"},
    }
    fixture = {
        "pageHrefs": [(memory / "wiki" / (n["id"] + ".md")).as_uri() for n in page_nodes],
        "pageSourceHrefs": [(memory / n["sources"][0]).as_uri() for n in page_nodes],
        "claimSourceHrefs": [(memory / "wiki" / n["sources"][0]).as_uri() for n in claim_nodes],
    }
    html = (Path(__file__).resolve().parents[1] / "templates" / "topology.html").read_text(encoding="utf-8")
    match = re.search(r"<script>(.*?)</script>", html, re.S)
    assert match is not None
    script = match.group(1).replace("%%GRAPH_DATA%%", tool_graph._script_json(payload)).replace(
        "%%MEMORY_BASE_PATH%%", tool_graph._script_json(memory.as_uri() + "/"))
    ...
    if inline:
        script += "document.getElementById('close-btn').onclick = function() {" + inline.group(1) + "};"
    ...
    result = subprocess.run([node, str(runner), str(script_path), str(fixture_path)],
                            capture_output=True, text=True, timeout=10)
    assert result.returncode == 0, result.stderr


@pytest.mark.parametrize("root_name", ["Graph Memory", "中文根", "literal%", "literal%20"])
def test_graph_initialized_exact_file_hrefs(tmp_path, root_name):
    _run_initialized_graph_check(tmp_path, r"""
...
const baseline = () => [settings.get('nodeColor')(other), settings.get('nodeOpacity')(other),
...
for (const action of ['close', 'background']) {
    ...
    const refreshes = setCounts.get('nodeOpacity');
    ...
    assert.ok(setCounts.get('nodeOpacity') > refreshes, action + ' must refresh renderer');
}
