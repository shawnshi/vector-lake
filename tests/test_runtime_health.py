...
def test_projection_drift_is_reported_but_does_not_block_its_own_repair(isolated_memory):
...
""",
    ...
    assert any("projection_drift" in item for item in health["degraded"])

    # The write path is the only repair channel, so drift must not block it.
    ok, message = execute_mutation_plan("Source_Blocked.md", content=_source_content("source_blocked", "Blocked Source"))
    assert ok is True
    assert "committed" in message.lower()


def test_hard_fault_still_blocks_canonical_writes(isolated_memory):
    _write_purpose_contract(isolated_memory)
    execute_mutation_plan("Source_Healthy.md", content=_source_content("source_healthy", "Healthy Source"))
    conn = db_store.get_connection()
    with db_store.transaction():
        conn.execute(
            "INSERT INTO mutation_outbox (filename, mutation_type, status, created_at) VALUES ('X.md', 'update', 'failed', '2026-01-01T00:00:00+00:00')"
        )

    health = assess_runtime_health()
    assert health["hard_ok"] is False
    with pytest.raises(RuntimeError, match="hard runtime fault"):
...
def test_watchdog_error_component_is_not_cleared_by_heartbeat(isolated_memory):
    ...
    assert health["ok"] is False
    assert health["hard_ok"] is True
    assert any("watchdog_unhealthy" in item for item in health["degraded"])
...
def test_deep_health_detects_equal_count_timeline_id_drift_without_blocking(isolated_memory):
    ...
    assert any("timeline_projection_drift" in item for item in health["degraded"])


def test_timeline_parity_stays_non_blocking_even_when_promoted(isolated_memory, monkeypatch):
    db_store.init_db()
    conn = db_store.get_connection()
    claim = {
    ...
    # A timeline rebuild is itself a write; blocking on this would deadlock.
    assert health["hard_ok"] is True
    assert any("timeline_projection_drift" in item for item in health["degraded"])
