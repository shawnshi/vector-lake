...
def test_query_payload_uses_run_scratch_and_prunes_stale_files(
    ...
    stale = payload_dir / "query_context_stale.md"
    ...
    payloads = list(payload_dir.glob("query_context_*.md"))
    assert stale.exists() is False
    assert len(payloads) == 1
    assert payloads[0].read_text(encoding="utf-8").find("memory evidence") >= 0
