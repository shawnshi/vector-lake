...
## 2023-11-09 - Precomputing Dictionary Fallbacks
**Learning:** In highly nested O(N^2) loops evaluating graph topological links (e.g. Vector Lake's `_calculate_weighted_edges`), performing string default `.get(key, "mentions")` followed by a dictionary lookup `pred_weights[pred]` inside the loop creates massive overhead due to repeated string evaluations and dictionary misses/hits.
**Action:** Pre-resolve expected strings into their final numeric float weights inside the outer O(N) loop and pre-fill the inner node's dictionary with a default weight for all known topological edges so the O(N^2) path can use a direct dictionary lookup `if key_b in triples_a: score += triples_a[key_b]`.
