...
- **`MEMORY/raw/`**: Source documents queue. Managed by system daemon, may be deleted/moved after ingestion.
...
- **`SQLite DB (vec_embeddings/operational_memory)`**: Machine-facing runtime memory and vectors compiled from claims. It is derived state, stored in SQLite, not hand-edited JSON.
...
3. **Review Items (Governance Queue)**: Contradictions, severe duplicates, or systemic knowledge gaps are detected asynchronously by Vector Lake daemons. Do NOT attempt to manually call non-existent MCP tools like enqueue_governance_item.
...
1. Build an Operational Memory Packet from SQLite Runtime before loading wiki pages.
