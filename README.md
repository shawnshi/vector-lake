...
- `VECTOR_LAKE_CANDIDATE_POOL`：进入重排的候选池规模，默认 `40`（原为 `max(40, top_k * 3)`）。池内来源类上限固定为 `int(pool * 0.6)`，结果窗口内来源类上限固定为 `tool_search.RESULT_SOURCE_CAP`（默认 `3`，原为 `max(1, int(top_k * 0.6))`）。三者均不随 `top_k` 缩放：**`top_k` 只决定窗口大小，不再决定谁有资格进窗口**。默认值下（`top_k=5`）行为与改动前逐位相同，这是回放验证过的。
