...
- runtime subagents 现只保留 2 个硬依赖角色：`vector-lake-ingestor` 和 `vector-lake-synthesizer`
...
CLI 子命令是主入口；`commands/` 目录里的 `.toml` 只是 Gemini 扩展侧的命令壳。当前它们按三层分组：

- Core workflow：`sync`、`search`、`query`、`lint`、`graph`、`review`、`delete`、`doctor`
- Governance workflow：`governance_migrate`、`governance_publish`、`governance_debt`、`governance_trace`、`governance_merge`、`governance_audit`
- Background services：`daemon_watchdog`
...
| `watchdog_sync.py` | 根目录薄入口，转发到 `vector_lake.watchdog_app` |
| `agents/vector-lake-ingestor` | ingest 两步 CoT 编译使用的 Gemini 子代理 |
| `agents/vector-lake-synthesizer` | query synthesis 使用的 Gemini 子代理 |
| `vector_lake/tools.py` | Tool facade，聚合各命令模块 |
| `vector_lake/ingest.py` | raw -> wiki 编译流程 |
| `vector_lake/indexer.py` | 生成 `index.json`，维护 page graph + claim graph 投影 |
| `vector_lake/review.py` | review queue 与治理队列兼容层 |
| `vector_lake/wiki_utils.py` | 路径、frontmatter、atomic write、meta store helper |
| `vector_lake/governance_store.py` | canonical object store 读写 |
| `vector_lake/claim_extractor.py` | page/block -> claim/evidence/source 抽取 |
| `vector_lake/change_sets.py` | change set 创建与发布包装 |
| `vector_lake/governance_metrics.py` | validity 推理与 debt metrics |
| `vector_lake/provenance.py` | provenance trace 构建 |
| `vector_lake/view_builder.py` | canonical objects -> markdown views |
| `vector_lake/tool_graph.py` | 3D 图谱渲染与 audit-graph |
...
- 能兼容旧工作流时，不破坏旧命令
- 对 Gemini 子代理只保留运行时硬依赖，不维持无执行链路的历史壳
