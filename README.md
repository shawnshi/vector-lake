...
- `MEMORY/wiki/.git`：Wiki 目录自身的历史仓库。它**不由 Vector Lake 管理**：没有 retention、没有配额、没有工具入口。它曾与 canonical 状态严重重复——2026-09-15 的重写从历史中移除了 **5.05 GiB** 已提交的库/备份/缓存状态（`.meta/vector_lake.db*`、`.meta/backups/**`、`.meta/embeddings.pkl`、`.meta/dump.sql`），`.git` 由 5.1 GB 降至 79 MB，并新增了 `wiki/.gitignore`（此前内层仓库没有 ignore 文件，而外层 `MEMORY/.gitignore` 的条目全部带 `wiki/` 前缀、在 wiki 仓库内永不匹配——这正是库被反复提交 7 次的原因）。当前占用仍以 `git count-objects -vH` 为准，不属 canonical 契约。
...
超过上限即停止版本删除。**操作要点（实测）**：`history-retention` 的预览与 apply 必须背靠背——运行中的 watchdog 会在约 2.5 分钟内改写候选集，导致 apply 以 `History retention candidate set changed after preview` 拒绝，而该入口**没有** `schema-migrate --confirm-no-writers` 式的停写开关；恢复游标只存在于 `history_retention_runs_v6.receipt_json.version_resume_cursors`（CLI 输出不含），且整场 campaign 必须固定同一个 `plan_as_of`（它是 `version_cursor_policy` 的组成部分）。同类并发风险也存在于维护型 wiki 写入：与 outbox cycle 并发时会留下一条 `failed` outbox 行 + 失效的投影配对（`canonical_generation_stale`），队列中没有待处理工作能自愈，修复代价是一次 `projection-rebuild-index --apply`（实测约 8 分钟）加 `recover_failed_mutation_outbox`。计划绑定参数、业务截止时间、运行代次、行级 guard 与持久
...
- `VECTOR_LAKE_QUERY_EMBEDDING`：查询时调用 embedding provider 的能力门。**默认为开**（未设置即启用）；只有精确设为 `1` 才启用，**显式设为 `0`（或任何非 `1` 值）即关闭**，因此宿主仍有 fail-closed 的关断方式。`GEMINI_API_KEY` 仍是独立的运行时能力检查，且 `_should_query_embedding` 只在词法检索不足时才触达 provider，所以普通检索保持本地延迟。
  **实测代价（活库、索引已填充）**：一旦触发 provider，单次检索约 **4,000 ms**（其中约 3,879 ms 为远程 `batchEmbedContents` 调用），而 FTS-only 热路径约 50 ms。因此「默认开」的实际含义是「**弱召回检索变准但变慢约 4 秒**」，不是「每次检索都变慢」；需要每次混合检索时才设 `VECTOR_LAKE_QUERY_EMBEDDING_ALWAYS=1`。要求永不外呼的宿主请设 `VECTOR_LAKE_QUERY_EMBEDDING=0`。
  开启前必须确认向量索引可用：`embedding_metadata_v8` 的 `(model, dimension, input_contract)` 必须与当前配置一致，否则拥挤前置条件 `total > metadata_invalid` 不成立，`_get_vector_search_results` 会立即返回 `{}` —— 此时开启只会外呼、烧 token 而拿不到任何候选。缺元数据的行被归类为 legacy 并成为 `embedding-backfill` 的候选（无需 `--include-existing`）；实测 7,176 节点约需 **74 个请求 / 7.5M tokens / 约 10 分钟**。
...
- `VECTOR_LAKE_BACKUP_MAX_TOTAL_BYTES`：maintenance 与 schema-migration 两个根的全局备份配额；默认 `0` 表示未配置并触发治理告警。未配置上限时 `quota_mode` 报为 `report`（请求值保留在 `requested_quota_mode`），因为未定义上限的 `enforce` 实际无法拦截任何写入。`VECTOR_LAKE_BACKUP_MIN_FREE_BYTES` / `VECTOR_LAKE_BACKUP_MIN_FREE_RATIO` 默认 `10 GiB` / `0.10`；`VECTOR_LAKE_BACKUP_QUOTA_MODE` 只接受 `enforce`（默认）或 `report`。**配额与保留下限存在实测不一致**：每份维护备份约 `3.82 GiB`，因此 `12 GiB` 配额下最多只能同时存在 3 份，而 retention 默认 `keep_latest = 5`（5 × 3.82 ≈ 19.1 GiB）**永不可达**；更关键的是 `backup_retention_maintenance` 无法在配额受压时自救（`--min-age-days` 要求 ≥1，且 `minimum_age` 规则保护所有当日备份），所以当日连做三次维护后，第四次的 `assert_backup_capacity` 会被拒绝且**没有受治理的出口**。原设计还会额外保护最新一份 `restorable` 备份（`latest_restorable` 守卫）。要配置大于 3 份的保留，必须同时抬高该配额。
...
- **自动摄取的两条 runner 都不能直接启用，但摄取通道可由宿主 subagent 服务。**
  `auto_ingest_config.json` 默认写 `runner: "host_relay"`；producer（`vector_lake/auto_ingest_runners/host_relay.py`）只把请求写入 `relay-spool/requests` 并轮询 `responses`，真正作答的是宿主侧 Pi 扩展 `extensions/host-relay-consumer/`。该扩展**默认不部署、未注册、且按其自身 README 必须保持关闭**：上游 `pi-subagents`（实测 **0.68.0** 仍然如此，不只是 0.67.0）的公开 `resolveSubagentLaunchContract` 在缺少 `sessionDir` 时返回 `host_required`，且公开委派没有创建子会话前校验的 digest/冻结契约句柄；该 README 明确禁止用私有 helper 或配置绕过。另一条 `codex_exec` 是 `AutoIngestConfig` 的**类默认**，但它是显式 **inert runner seam**（模块 docstring：「the controller switch is deferred until a separate byte-equivalence corpus has been reviewed and is empty」），且活配置里 8 个 pin 全为空。两条都不能启用，因此**不得把 `enabled` 置为 `true`**：每次启动会预留 `max_tokens_per_task`（默认 262144）并在 `timeout_seconds`（默认 1200 s，实测约 32.7 min）后以 `relay_response_timeout` 失败、连续 `max_consecutive_infra_failures` 次后熔断，并把 job 留在 `awaiting_subagent` 直到 24 h 后过期清扫。
  **支持的手动路径（不需要 codex、不需要 Pi 扩展、宿主无关，已实测端到端跑通）：**
  1. `python cli.py ingest-tasks --claim --limit 1 --lease-seconds 3600`（自动摄取关闭时才允许；返回 `job_id`、`task_packet`、`task_packet_path`、`lease_owner`/`lease_token`/`lease_generation`）。
  2. 读 `task_packet_path`：其 `prompt`（约 44 KB）是**约束性简报**（schema/governance + 摄取规则 + 末尾 6 问自检）；末尾 `[CURRENT-ENVIRONMENT SUBAGENT HANDOFF]` 给出输出契约。
  3. 读 `task_packet.metadata.processed_data.filepath` 指向的原文（任务包**不含**原文，由宿主读取）。
  4. 交给**宿主自身的 subagent** 生成：一个 JSON 数组，元素为 `{"filename", "content"}`，`content` 是含 YAML frontmatter 的完整 Markdown；数组必须**恰好包含一个** `filename` 等于 `processed_data.canonical_name` 的 `Source_*.md` 页面。
  5. `processed_data` = 任务包的 `metadata.processed_data` + `job_id` + `lease_owner`/`lease_token`/`lease_generation` + `integration`。`integration.disposition` 取 `integrated`/`standalone`/`rejected`：`integrated` 的 `relations` 必须复用 `metadata.processed_data.integration_candidates` 里**原样的** `target`/`target_hash`/`target_projection_hash`（不得自造哈希），其余两种需可审计的 `reason`。
  6. **先只读预检再提交**：`_prepare_final_ingest_files` **不够**（它比真实门禁弱），必须再跑 `_validate_final_ingest_files`；否则会像实测那样在 `finalize_ingest` 才被 `defense_hook`/purpose 契约拒绝（好消息：拒绝**不消耗** `retries`，lease 仍可复用）。
  7. `finalize_ingest(files, processed_data)`。
  **已知挡位**：`integrated` 若命中一个 frontmatter 的 `categories` 不是「恰好一个域的列表」的 legacy 目标页，维护容错路径会以 `categories must be a list with exactly one domain` 拒绝整个摄取（实测：`Policy_检查检验结果互认规范.md` 的 `categories:` 为空）；此时用 `standalone` 发布源页面，或先按契约修复该目标页。
  运行期仍可用 `relay_spool_unconsumed:requests=N:responses=0` 这一 warning 判断 spool 是否在产生无人消费的请求；过期清扫留下的终态失败会带精确隔离标记（以 `auto_ingest_quarantined_jobs` 可见，不再阻断写闸门），其恢复入口是先释放遗留的 change-set 幂等键（如有），再 `python cli.py ingest-tasks --repair-debt --limit 0`（preview）与 `--apply --limit 0`。
  `auto_ingest_config.json` 不支持环境变量覆盖，也没有 profile 键；它就是 `<meta>/auto_ingest_config.json`。配置解析 fail-closed：`enabled: true` 时 `allow_model_processing_raw_text` 必须同时为 `true`。失效的熔断窗口会在下一次 tick 自行关闭并清零计数（`_reconcile_breaker_state`），包括 `enabled: false` 的 tick —— 健康面读取该 ledger 与启用状态无关。
- `VECTOR_LAKE_SQLITE_CACHE_KIB`：每个 SQLite 连接的页缓存，默认 `65536`（64 MiB），下限 `2000`（SQLite 自身默认 2 MiB）、上限 `524288`（512 MiB；`cache_size` 是**每连接**设置，该上限同时界定多连接合计占用）。此前从未设置，一个多 GiB 的 canonical 库一直跑在 2 MiB 缓存上。实测于 3.6 GiB 实库：`PRAGMA quick_check` 从 **62.6 s 降到 16.1 s（约 3.9×）**，全语料 FTS 摘要扫描 391→280 ms；64 MiB 与 256 MiB 差别很小，因此默认取 64 MiB。**该设置不改善首次检索墙钟**——首次检索的主要成本是索引物化（844 个对象文件 + canonical 字节校验，约 1.1 s），与页缓存无关。
- `VECTOR_LAKE_SQLITE_MMAP_BYTES`：SQLite `mmap_size`，默认 `0`（关闭）。SQLite 明确要求内存映射文件不被其它进程截断，而本部署由多个宿主适配器共享同一数据库，因此默认关闭、按主机显式开启；请求正数但未被应用时 fail-closed 报错，而不是静默退回默认值。
- `VECTOR_LAKE_STORAGE_OBJECT_SCAN_MAX_FILES`：每日存储增长采样遍历投影对象库的文件上限，默认 `2000000`、范围 `1000..50000000`。此前硬编码 `200000` 已低于实库（228,636 个文件），于是 `projection_object_bytes` / `projection_orphan_bytes` 被静默冻结在真实总量之下（漏报孤儿存储约 0.9 GB），而 `projection_object_scan_complete=false` 没有任何告警；现在该上限与 GC 的 `VECTOR_LAKE_PROJECTION_GC_MAX_SCAN_FILES` 同量级，且扫描不完整会作为 warning 出现在 Runtime Health。代价是采样从“截断后立即返回”变为完整遍历，实库上单次约 59 s。
- Runtime Health 对三类“缺席即故障”的零值给出显式信号：`maintenance_backup_absent`（备份根清单完整、策略不是 `skip`、备份为 0 字节且数据库超过 `VECTOR_LAKE_BACKUP_ABSENCE_MIN_DATABASE_BYTES`，默认 `67108864` 字节）；`auto_ingest_circuit_tripped`（连续基础设施失败数达到 `max_consecutive_infra_failures`，即使冷却窗口恰好已过——此前“已跳闸但窗口关闭”与健康状态在状态面上完全同形）；`projection_object_scan_incomplete`（对象库扫描被上限截断，增长趋势因此不可信）；`subagent_lease_expired:N`（存在 `subagent_processing` 且租约已过期的 ingest job——`expire_stale_subagent_jobs` 只扫 `awaiting_subagent`、`release_ingest_subagent_task_claim` 要求未过期租约，因此死 owner 既不回收也不报告；实测滞留 38 h。它仍可被 `claim_subagent_jobs` 认领，所以只为 warning，恢复路径是先 claim 再 release 回 `awaiting_subagent`，**不消耗 `retries`**）。`VECTOR_LAKE_RELAY_SPOOL_SCAN_LIMIT` 为 relay spool 计数上限，默认 `5000`，超限时 `relay_spool.scan_complete=false`。
- `VECTOR_LAKE_LOG_MAX_BYTES` / `VECTOR_LAKE_LOG_BACKUP_COUNT`：MCP server、watchdog 与 CLI 启动时安装的轮转文件日志，默认 `8388608`（8 MiB）× `3`，上限 `536870912` × `20`。日志写入 `<meta>/logs/vector-lake.log`，同时保留 stderr；**绝不安装 stdout handler**（MCP 走 stdio，stdout 污染会破坏协议）。此前只调用 `logging.basicConfig`，其中 MCP server 用 `force=True` 覆盖先前导入的 handler，且全盘没有任何 `*.log`，于是“持续失败的 watchdog”只留下陈旧的 JSON 状态、没有可追踪痕迹。
